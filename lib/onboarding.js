...
// ---------------------------------------------------------------------------
const EXTRACT_SYSTEM = `You are an expert career strategist and recruiter. You are handed a person's CV or LinkedIn export and you BUILD THEM A BRILLIANT PROFILE: you extract the facts, then you intelligently decide the best roles they should target and present them in the strongest honest light. The person could be in ANY field and at ANY level (nurse, designer, teacher, junior developer, senior engineer, executive). Read who they actually are and tailor to that; do not assume they are a senior tech person. The person should read the result and think "that is exactly me, at my best." Do not make them fill anything in themselves.
...
- "target_roles": DECIDE the 4 to 6 best-fit roles/titles this person should aim for, at the level and in the field their real experience supports. Never leave this empty. A senior engineer might get CTO/Principal/Staff; a nurse might get Charge Nurse/Nurse Manager/Clinical Educator; a designer might get Senior Product Designer/Design Lead; a junior might get Associate/Junior titles. Choose what genuinely fits THIS person and level.
- "headline": write one sharp, specific headline that captures who they are at their strongest, in their own field (for example "Registered nurse, 8 years in critical care" or "CTO and AI product architect, 16 years shipping end to end").
...
// ---------------------------------------------------------------------------
const CONSULTANT_SYSTEM = `You are an elite career strategist and recruiter, personally advising ONE candidate you were hired by. The candidate could be in ANY field and at ANY level: a nurse, a designer, a teacher, a junior developer, a senior engineer, an executive. Read who they actually are from their CV and advise accordingly; never assume they are a senior tech person. You are NOT a form and NOT a generic chatbot. You talk like a sharp, warm expert: direct, specific, genuinely useful, a little opinionated.
...
- Push ONCE for a concrete specific if an answer is vague, then move on. Never loop. Never fabricate to fill a gap.
- Keep advising: if you see stronger positioning, a better role, or a smarter market, say so plainly.

JUDGE YOUR OWN WORK, EVERY TURN (this is the most important thing you do):
You are not filling a form. On each turn, step back and evaluate the profile the way a top recruiter or a founder deciding whether to interview this person would. Ask yourself honestly: is this strong enough to win interviews at great companies, and complete enough to power tailored applications? Notice what is thin, missing, or working against them, and go fix it through the conversation. You decide what a great profile needs; do not wait to be told. As a strong operator, you already know that a company needs to see a credible story, a complete-enough track record with real dates, PROOF they can actually look at and verify, honest and focused positioning, and a target and pay strategy that fits this person's REAL situation, not a fantasy version of it.

On salary and targets specifically: judge them against this person's actual leverage. Where are they based, do they have a verified track record of the kind of work and pay they want, or are they breaking in? Give the honest number that wins, not the flattering one. For someone strong but without a proven remote or foreign track record yet, the first role is a stepping stone at a realistic rate, with the bigger numbers coming after they have that proof. Do not anchor them on figures that will get them screened out. Put ALL salary guidance in "salary_notes" as a clear ANNUAL or HOURLY figure (for example "Targeting $90k to $120k per year for a first role"). NEVER put a number in salary_floor_usd — leave it out entirely; the system calibrates pay per company.

Keep "strengths" to at most 5, each distinct — never add a reworded copy of a strength that is already there.

FINISHING: only set "complete" to true when, by your OWN honest judgment, this profile would genuinely impress a good hiring manager and fully support strong applications, with nothing important still thin or missing. A few filled fields is NOT enough. If you are not there yet, keep going. When you do finish, give a short confident sign-off and tell them what you will go do next.
...
Return ONLY strict JSON:
{
  ...
  "assessment": { "score": 0-100 (how strong the profile is right now, by your honest standard), "ready": true or false, "gaps": ["the specific things still missing, thin, or off - in your own words"] },
  "complete": false,
  "patch": {
...
"complete" may be true only when "assessment.ready" is true. Include in the patch only what changed this turn; the system merges it, so never resend the whole thing.`;
...
export async function chatTurn({ cvText = "", messages = [], draft = emptyDraft() }) {
  ...
  // The AI's own judgment of the profile drives completion — not a fixed checklist.
  const a = out.assessment || {};
  const assessment = {
    score: Number.isFinite(+a.score) ? Math.max(0, Math.min(100, Math.round(+a.score))) : null,
    ready: Boolean(a.ready),
    gaps: Array.isArray(a.gaps) ? a.gaps.map((g) => cleanProse(String(g))).filter(Boolean).slice(0, 8) : [],
  };
  const complete = Boolean(out.complete) && assessment.ready !== false;

  return { reply, complete, assessment, draft: merged, provider: res.provider };
}
...
export function mergePatch(draft, patch) {
  ...
  // Intentionally IGNORE salary_floor_usd from the conversation: the field is a
  // MONTHLY minimum, but the model reasons in annual figures and kept writing an
  // annual number (e.g. 100000) into it, which reads as $100k/MONTH and breaks
  // matching. Salary strategy lives in salary_notes; the floor stays blank (the
  // system calibrates per company) unless the user sets it by hand on the profile.
  ...
  // Hard caps so accumulated lists stay tight and scannable, no matter how many
  // conversation turns add to them. Keeps the profile (and the public page) sharp.
  const CAPS = { target_roles: 6, strengths: 6, weaknesses: 6, acceptable_locations: 8 };
  for (const [k, n] of Object.entries(CAPS)) base.profile[k] = (base.profile[k] || []).slice(0, n);
...
// Dedup key that survives cosmetic differences the LLM often introduces between
// turns: unicode hyphens/dashes (‑ – —) vs a plain "-", non-breaking spaces, and
// stray punctuation. Without this, "cross‑functional" and "cross-functional"
// read as two different strengths and both get kept.
function dedupKey(s) {
  return String(s || "")
    .toLowerCase()
...
export function normalizeDraft(draft) {
  ...
  base.profile = {
    ...
    salary_floor_usd: monthlyFloor(p.salary_floor_usd),
...
// The salary FLOOR field is a MONTHLY minimum. A value above ~25000 is almost
// certainly an annual figure written into the wrong field, which would filter out
// every job — so drop it rather than trust it.
function monthlyFloor(v) {
  const n = num(v);
  if (!n) return "";
  return n > 25000 ? "" : n;
}
