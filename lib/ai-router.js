...
const PROVIDERS = [
  {
    // PRIMARY — OpenRouter with a paid model (real quota = no "AI is busy").
    // Swap the model anytime with one env var: OPENROUTER_MODEL. Cheap default
    // (DeepSeek V4 Flash) for dev + bulk; set to openai/gpt-5.5 or
    // anthropic/claude-sonnet-5 for premium writing when you want top polish.
    name: "openrouter",
    priority: 1,
    ...
    call: (cfg, payload) =>
      callOpenAICompatible(
        ...
        // Disable chain-of-thought: we want a direct answer, not reasoning. Some
        // OpenRouter models (DeepSeek V4 Flash) otherwise burn the token budget
        // "thinking" and return null content. This makes calls fast + reliable.
        { reasoning: { enabled: false } }
      ),
  },
  {
    // Free fallback #1 — used if OpenRouter is ever unreachable.
    name: "gemini",
    priority: 2,
...
async function callOpenAICompatible(baseUrl, { apiKey, model }, { prompt, system, json, temperature, maxTokens }, extraHeaders = {}, extraBody = {}) {
  const messages = [];
  ...
  // Reasoning models (e.g. DeepSeek V4 Flash) can return the answer in `reasoning`
  // with `content` null/empty — salvage it so a reasoning turn is not lost. Our
  // JSON parser extracts the object from whichever field carries it.
  const text = msg.content && String(msg.content).trim() ? msg.content : msg.reasoning || "";
