...
export async function callAI(opts = {}) {
  ...
  for (const provider of ordered) {
    ...
    try {
      ...
      // Retry only transient (network) errors; 429/5xx fail over to the next
      // provider rather than hammering a hurting one.
...
// HELPERS
// ---------------------------------------------------------------------------
async function retry(fn, { tries = 2, retryOn = () => true } = {}) {
  let lastErr;
