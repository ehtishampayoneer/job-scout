...
export async function runScoutForUser(userId, supabase, { maxJobs = 100, maxScore = 20 } = {}) {
  ...
  // Hard filter + dedupe. Two dedupe keys:
  //  - source|url  : exact same posting fetched twice
  //  - company|title: the SAME role surfaced by different sources (e.g. a
  //    company's Greenhouse form AND Adzuna AND WeWorkRemotely). Critical now
  //    that many sources overlap. First-seen wins, and company/ATS sources are
  //    fetched before aggregators, so the more-direct listing is the one kept.
  const seen = new Set();
