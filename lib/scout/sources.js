...
import { themuse, himalayas, jsearch, adzuna, jooble } from "./aggregators";
import { logger } from "@/lib/log";

const firstStr = (v) => (Array.isArray(v) ? v[0] : v) || "";

// `queries` are the candidate's target-role search terms, used by the
// query-based aggregators (JSearch/Adzuna/Jooble). Firehose sources ignore them.
export async function fetchAllSources({ queries = [] } = {}) {
  const groups = await Promise.allSettled([
    ...
    leverAll(),
    smartrecruitersAll(),
    // Cross-platform aggregators (keyless + key-gated). See aggregators.js.
    themuse(),
    himalayas(),
    jsearch(queries),
    adzuna(queries),
...
// Boards are fetched in PARALLEL so a large curated list stays fast.
async function greenhouseBoard(token) {
  const out = [];
  try {
    // No content=true: keeps the payload tiny (title + location + url) so we can
    // pull many boards fast without blowing memory. Enough to filter and rank;
    // the full posting is one click away at the URL.
    ...
    for (const j of data?.jobs || []) {
      ...
      out.push({
        ...
        company: token,
        title: decodeEntities(j.title).trim(),
        raw_text: clip(`${decodeEntities(j.title)}. ${loc} at ${token}.`, 500),
        salary_range: null,
        location_type: locType(loc),
        location_raw: loc,
      });
...
export async function greenhouseAll() {
  const groups = await Promise.allSettled(GREENHOUSE_BOARDS.map(greenhouseBoard));
  return groups.flatMap((g) => (g.status === "fulfilled" ? g.value : []));
}

// --- Ashby (public job-board JSON) ---------------------------------------
async function ashbyBoard(org) {
  const out = [];
  try {
...
export async function ashbyAll() {
  const groups = await Promise.allSettled(ASHBY_BOARDS.map(ashbyBoard));
  return groups.flatMap((g) => (g.status === "fulfilled" ? g.value : [])).filter((j) => j.url);
}
