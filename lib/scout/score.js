...
- Company quality: a real, reputable, well-regarded employer in the candidate's own field raises the score; a great company with a merely-good-fit role still beats a perfect-title role at a weak or sketchy company.
...
Lower the trust score for: vague descriptions with no company specifics, upfront-fee or pay-for-training language, requests for money or personal financial details, only a free personal email as contact, or unrealistic pay. Empty array if nothing is suspicious.`;
  ...
  // Generous token budget: DeepSeek V4 Flash is a reasoning model and will
  // truncate to null content if it runs out of room mid-reasoning on a long job
  // description. 1500 leaves room to reason AND emit the JSON.
  const res = await callAI({ system: SYSTEM, prompt, json: true, temperature: 0.3, maxTokens: 1500 });
  const j = res.json || {};
  return {
